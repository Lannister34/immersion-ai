...
export function read(image: Buffer): string {
  ...
  const ccv3Index = textChunks.findIndex((chunk: { keyword: string }) => chunk.keyword.toLowerCase() === 'ccv3');
  if (ccv3Index > -1) {
    return Buffer.from(textChunks[ccv3Index].text, 'base64').toString('utf8');
  }

  const charaIndex = textChunks.findIndex((chunk: { keyword: string }) => chunk.keyword.toLowerCase() === 'chara');
  if (charaIndex > -1) {
